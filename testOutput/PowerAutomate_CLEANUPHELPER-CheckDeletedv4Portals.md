# Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Portals

**Generated on:** 2025-07-15 19:14:48
**Flow ID:** 000D3A3411D9
**Source File:** CLEANUPHELPER-CheckDeletedv4Portals-ABB45C39-A794-EE11-BE37-000D3A3411D9.json
...
### Summary
- **Flow Name:** CLEANUPHELPER-CheckDeletedv4Portals
- **Triggers:** 1
- **Actions:** 1 1 1
- **Connections:** 3
- **Parameters:** 1
...
@startuml
!theme plain
title Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Portals
   ...
   if (condition) then (yes)
    ...
    :Deleted_Portals (Scope);
     if (condition) then (yes)
      ...
      :DeletedPortals (Query);
      :Get_Actual__Deleted_Portals (Scope);
       if (condition) then (yes)
        :Select_Actual__Deleted_Portals (Select);
        :Parse_Actual__Deleted_Portals (ParseJson);
       endif
       partition "Get_Actual__Deleted_Portals" {
        :Select_Actual__Deleted_Portals (Select);
        :Parse_Actual__Deleted_Portals (ParseJson);
       }
      :Get_Inventory__Deleted_Portals (Scope);
       if (condition) then (yes)
        :Select_Inventory__Deleted_Portals (Select);
        :Parse_Inventory__Deleted_Portals (ParseJson);
       endif
       partition "Get_Inventory__Deleted_Portals" {
        :Select_Inventory__Deleted_Portals (Select);
        :Parse_Inventory__Deleted_Portals (ParseJson);
       }
     endif
     partition "Deleted_Portals" {
      :Mark_deleted (Foreach);
       if (condition) then (yes)
        :Find_it_for_GUID (OpenApiConnection);
        :If_found_mark_deleted (If);
    ...
    :Get_Basics (Scope);
     if (condition) then (yes)
      :Get_Portals_and_Exit_if_no_permissions (Scope);
       if (condition) then (yes)
       ...
       partition "Get_Portals_and_Exit_if_no_permissions" {
        :List_Envt_Portals (OpenApiConnection);
        :Terminate__no_access (Scope);
         if (condition) then (yes)
          :Respond_to_a_PowerApp_for_fail_for_no_access (Response);
