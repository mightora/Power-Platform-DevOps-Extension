# Power Automate Flow: CLEANUPHELPER-CheckDeletedv4BusinessProcessFlows

**Generated on:** 2025-07-15 19:14:48
**Flow ID:** 000D3A341FFF
**Source File:** CLEANUPHELPER-CheckDeletedv4BusinessProcessFlows-2A285D04-4C94-EE11-BE37-000D3A341FFF.json
...
### Summary
- **Flow Name:** CLEANUPHELPER-CheckDeletedv4BusinessProcessFlows
- **Triggers:** 1
- **Actions:** 1 1 1
- **Connections:** 3
- **Parameters:** 1
...
@startuml
!theme plain
title Power Automate Flow: CLEANUPHELPER-CheckDeletedv4BusinessProcessFlows
 ...
 if (condition) then (yes)
  ...
  :Run_Check_Deleted_Scope (Scope);
   if (condition) then (yes)
    ...
    :Get_Basics (Scope);
     if (condition) then (yes)
      :Get_BPFs_and_Exit_if_no_permissions (Scope);
       if (condition) then (yes)
       ...
       partition "Get_BPFs_and_Exit_if_no_permissions" {
        :List_Envt_BPFs (OpenApiConnection);
        :Terminate__no_access (Scope);
         if (condition) then (yes)
          :Respond_to_a_PowerApp_for_fail_for_no_access (Response);
    ...
    :Deleted_BPFs (Scope);
     if (condition) then (yes)
      :Get_Inventory__Deleted_BPFs (Scope);
       if (condition) then (yes)
        :Select_Inventory__Deleted_BPFs (Select);
        :Parse_Inventory__Deleted_BPFs (ParseJson);
       endif
       partition "Get_Inventory__Deleted_BPFs" {
        :Select_Inventory__Deleted_BPFs (Select);
        :Parse_Inventory__Deleted_BPFs (ParseJson);
       }
      :Get_Actual__Deleted_BPFs (Scope);
       if (condition) then (yes)
        :Select_Actual__Deleted_BPFs (Select);
        :Parse_Actual__Deleted_BPFs (ParseJson);
       endif
       partition "Get_Actual__Deleted_BPFs" {
        :Select_Actual__Deleted_BPFs (Select);
        :Parse_Actual__Deleted_BPFs (ParseJson);
       }
      :DeletedBPFs (Query);
