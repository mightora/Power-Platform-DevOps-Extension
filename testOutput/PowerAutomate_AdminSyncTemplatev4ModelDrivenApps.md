# Power Automate Flow: AdminSyncTemplatev4ModelDrivenApps

**Generated on:** 2025-07-15 19:14:47
**Flow ID:** 000D3A341FFF
**Source File:** AdminSyncTemplatev4ModelDrivenApps-7C7AC6E2-1B7C-EE11-8179-000D3A341FFF.json
...
### Summary
- **Flow Name:** AdminSyncTemplatev4ModelDrivenApps
- **Triggers:** 1
- **Actions:** 1 1 1 1 1 1
- **Connections:** 2
- **Parameters:** 6
...
@startuml
!theme plain
title Power Automate Flow: AdminSyncTemplatev4ModelDrivenApps

start
:Trigger: When_a_row_is_added_or_modified (OpenApiConnectionWebhook);\n:Check_if_Model_Driven_Apps_can_and_should_be_retrieved_for_this_environment (If);
 if (condition) then (yes)
  :Terminate_inquiry_not_supported (Terminate);
 endif
:Initialize_mdasToInvenotry (InitializeVariable);
:Inventory_Model_Driven_Apps_MDAs_for_this_envt (Scope);
 if (condition) then (yes)
  :Get_Basics (Scope);
   if (condition) then (yes)
    :Get_MDAs_and_Exit_if_no_MDAs_in_envt_or_if_no_permissions (Scope);
     if (condition) then (yes)
      :Count_MDAs (If);
       if (condition) then (yes)
        :Quit_if_no_MDAs (Terminate);
       endif
      :Terminate_if_no_access (Terminate);
      :List_Envt_MDAs (OpenApiConnection);
     endif
     partition "Get_MDAs_and_Exit_if_no_MDAs_in_envt_or_if_no_permissions" {
      :Count_MDAs (If);
       if (condition) then (yes)
        :Quit_if_no_MDAs (Terminate);
       endif
    ...
    :List_Inventory_MDAs (OpenApiConnection);
   endif
  ...
  :Look_for_MDAs_to_Inventory (Scope);
   if (condition) then (yes)
    :Look_for_new_MDAs (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_MDAs (Scope);
       if (condition) then (yes)
        :Select_Inventory__New_MDAs (Select);
        :Parse_Inventory__New_MDAs (ParseJson);
       endif
       partition "Get_Inventory__New_MDAs" {
        :Select_Inventory__New_MDAs (Select);
        :Parse_Inventory__New_MDAs (ParseJson);
       }
      :Get_Actual__New_MDAs (Scope);
       if (condition) then (yes)
        :Select_Actual__New_MDAs (Select);
        :Parse_Actual__New_MDAs (ParseJson);
       endif
       partition "Get_Actual__New_MDAs" {
        :Select_Actual__New_MDAs (Select);
        :Parse_Actual__New_MDAs (ParseJson);
       }
    ...
    :If_full_inventory_run (If);
     if (condition) then (yes)
      ...
      :Look_for_rpas_manually_flagged_for_inventory (Scope);
       if (condition) then (yes)
        :MDAsToInventory__ManualRequest (OpenApiConnection);
        :Add_manual_requests_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_mdasToInventory__Manual_Requests (AppendToArrayVariable);
         endif
         repeat
          :Append_to_mdasToInventory__Manual_Requests (AppendToArrayVariable);
         repeat while (more items)
      ...
      :Look_for_ones_modified_since_date (Scope);
       if (condition) then (yes)
        ...
        :Filter_to_mdas_modified_since_Get_past_time (Query);
        :Add_each_recently_modified_to_array (Foreach);
         if (condition) then (yes)
          :Get_MDA_from_Inventory (OpenApiConnection);
   ...
   partition "Look_for_MDAs_to_Inventory" {
    :Look_for_new_MDAs (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_MDAs (Scope);
       if (condition) then (yes)
     ...
     if (condition) then (yes)
      ...
      :Object_Found_so_add_or_update_inventory (Scope);
       if (condition) then (yes)
        ...
        :Upsert_Record (If);
         if (condition) then (yes)
          :Upsert_Model_Driven_App_Orphan (OpenApiConnection);
         endif
         else (no)
          :Upsert_Model_Driven_App (OpenApiConnection);
         endif
       endif
 ...
 partition "Inventory_Model_Driven_Apps_MDAs_for_this_envt" {
  :Get_Basics (Scope);
   if (condition) then (yes)
    :Get_MDAs_and_Exit_if_no_MDAs_in_envt_or_if_no_permissions (Scope);
     if (condition) then (yes)
...
|-------------|------|-------------|
| Check_if_Model_Driven_Apps_can_and_should_be_retrieved_for_this_environment | If | Operation ID: 9b6688e4-c25b-4e40-9deb-fcf181094357 |
| Initialize_mdasToInvenotry | InitializeVariable | Operation ID: b133bb41-c98a-45c4-a8f4-fbf40b3c3873 |
| Inventory_Model_Driven_Apps_(MDAs)_for_this_envt | Scope | Operation ID: 7fabdbe3-cca0-4e6b-9f0b-b9ac39bea0b9 |
