# Power Automate Flow: AdminSyncTemplatev4BusinessProcessFlows

**Generated on:** 2025-07-15 19:14:47
**Flow ID:** 000D3A3411D9
**Source File:** AdminSyncTemplatev4BusinessProcessFlows-6B10220E-5494-EE11-BE37-000D3A3411D9.json
...
### Summary
- **Flow Name:** AdminSyncTemplatev4BusinessProcessFlows
- **Triggers:** 1
- **Actions:** 1 1 1 1 1 1
- **Connections:** 2
- **Parameters:** 5
...
@startuml
!theme plain
title Power Automate Flow: AdminSyncTemplatev4BusinessProcessFlows

start
:Trigger: When_a_row_is_added_or_modified (OpenApiConnectionWebhook);\n:Check_if_Business_Process_Flows_can_and_should_be_retrieved_for_this_environment (If);
 if (condition) then (yes)
  :Terminate_inquiry_not_supported (Terminate);
 endif
:Initialize_bpfsToInvenotry (InitializeVariable);
:Inventory_Business_Process_Flows_BPFs_for_this_envt (Scope);
 if (condition) then (yes)
  :Look_for_BPFs_to_Inventory (Scope);
   if (condition) then (yes)
    :Look_for_new_BPFs (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_BPFs (Scope);
       if (condition) then (yes)
        :Select_Inventory__New_BPFs (Select);
        :Parse_Inventory__New_BPFs (ParseJson);
       endif
       partition "Get_Inventory__New_BPFs" {
        :Select_Inventory__New_BPFs (Select);
        :Parse_Inventory__New_BPFs (ParseJson);
       }
      :Get_Actual__New_BPFs (Scope);
       if (condition) then (yes)
        :Select_Actual__New_BPFs (Select);
        :Parse_Actual__New_BPFs (ParseJson);
       endif
       partition "Get_Actual__New_BPFs" {
        :Select_Actual__New_BPFs (Select);
        :Parse_Actual__New_BPFs (ParseJson);
       }
      :Add_new_ones_to_inventory_array (Scope);
       if (condition) then (yes)
        ...
        :Add_new_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_bpfsToInventory__new_bpfs (AppendToArrayVariable);
         endif
         repeat
          :Append_to_bpfsToInventory__new_bpfs (AppendToArrayVariable);
    ...
    :If_full_inventory_run (If);
     if (condition) then (yes)
      :Add_every_inventoried_bpf_to_bpfsToInventory (Foreach);
       if (condition) then (yes)
        :Append_to_bpfsToInventory__full_inventory (AppendToArrayVariable);
       endif
       repeat
        :Append_to_bpfsToInventory__full_inventory (AppendToArrayVariable);
       repeat while (more items)
      ...
      :Look_for_bpfs_manually_flagged_for_inventory (Scope);
       if (condition) then (yes)
        :BPFsToInventory__ManualRequest (OpenApiConnection);
        :Add_manual_requests_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_bpfsToInventory__Manual_Requests (AppendToArrayVariable);
         endif
         repeat
          :Append_to_bpfsToInventory__Manual_Requests (AppendToArrayVariable);
         repeat while (more items)
       endif
       partition "Look_for_bpfs_manually_flagged_for_inventory" {
        :BPFsToInventory__ManualRequest (OpenApiConnection);
        :Add_manual_requests_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_bpfsToInventory__Manual_Requests (AppendToArrayVariable);
      ...
      :Look_for_ones_modified_since_date (Scope);
       if (condition) then (yes)
        ...
        :Filter_to_bpfs_modified_since_Get_past_time (Query);
        :Add_each_recently_modified_to_array (Foreach);
         if (condition) then (yes)
          :Get_BPF_from_Inventory (OpenApiConnection);
   ...
   partition "Look_for_BPFs_to_Inventory" {
    :Look_for_new_BPFs (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_BPFs (Scope);
       if (condition) then (yes)
     ...
     if (condition) then (yes)
      ...
      :Object_Found_so_add_or_update_inventory (Scope);
       if (condition) then (yes)
        ...
        :Upsert_Record (If);
         if (condition) then (yes)
          :Upsert_BPF_Orphaned (OpenApiConnection);
         endif
         else (no)
          :Upsert_BPF (OpenApiConnection);
         endif
       endif
  ...
  :Temp_clean_up (Scope);
   if (condition) then (yes)
    :List_BPFs_with_missing_mandatory_fields (OpenApiConnection);
    :Clean_up_BPFs_with_missing_mandatory_fields (Foreach);
     if (condition) then (yes)
      :Clean_up_BPF_with_missing_mandatory_fields (OpenApiConnection);
     endif
     repeat
      :Clean_up_BPF_with_missing_mandatory_fields (OpenApiConnection);
     repeat while (more items)
   endif
   partition "Temp_clean_up" {
    :List_BPFs_with_missing_mandatory_fields (OpenApiConnection);
    :Clean_up_BPFs_with_missing_mandatory_fields (Foreach);
     if (condition) then (yes)
      :Clean_up_BPF_with_missing_mandatory_fields (OpenApiConnection);
  ...
  :Get_Basics (Scope);
   if (condition) then (yes)
    :Get_BPFs_and_Exit_if_no_BPFs_in_envt_or_if_no_permissions (Scope);
     if (condition) then (yes)
      :Count_BPFs (If);
       if (condition) then (yes)
        :Quit_if_no_BPFs (Terminate);
       endif
      :Terminate_if_no_access (Terminate);
      :List_Envt_BPFs (OpenApiConnection);
     endif
     partition "Get_BPFs_and_Exit_if_no_BPFs_in_envt_or_if_no_permissions" {
      :Count_BPFs (If);
       if (condition) then (yes)
        :Quit_if_no_BPFs (Terminate);
       endif
    ...
    :List_Inventory_BPFs (OpenApiConnection);
   endif
 ...
 partition "Inventory_Business_Process_Flows_BPFs_for_this_envt" {
  :Look_for_BPFs_to_Inventory (Scope);
   if (condition) then (yes)
    :Look_for_new_BPFs (Scope);
     if (condition) then (yes)
...
|-------------|------|-------------|
| Check_if_Business_Process_Flows_can_and_should_be_retrieved_for_this_environment | If | Operation ID: 9b6688e4-c25b-4e40-9deb-fcf181094357 |
| Initialize_bpfsToInvenotry | InitializeVariable | Operation ID: b133bb41-c98a-45c4-a8f4-fbf40b3c3873 |
| Inventory_Business_Process_Flows_(BPFs)_for_this_envt | Scope | Operation ID: 7fabdbe3-cca0-4e6b-9f0b-b9ac39bea0b9 |
