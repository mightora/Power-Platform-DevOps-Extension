# Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Desktopflows

**Generated on:** 2025-07-15 19:14:48
**Flow ID:** 000D3A341D27
**Source File:** CLEANUPHELPER-CheckDeletedv4Desktopflows-FB942241-A794-EE11-BE37-000D3A341D27.json
...
### Summary
- **Flow Name:** CLEANUPHELPER-CheckDeletedv4Desktopflows
- **Triggers:** 1
- **Actions:** 1 1 1
- **Connections:** 3
- **Parameters:** 1
...
@startuml
!theme plain
title Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Desktopflows
 ...
 if (condition) then (yes)
  ...
  :Run_Check_Deleted_Scope (Scope);
   if (condition) then (yes)
    ...
    :Get_Basics (Scope);
     if (condition) then (yes)
      :Get_RPAs_and_Exit_if_no_permissions (Scope);
       if (condition) then (yes)
       ...
       partition "Get_RPAs_and_Exit_if_no_permissions" {
        :List_Envt_Desktop_Flows (OpenApiConnection);
        :Terminate__no_access (Scope);
         if (condition) then (yes)
          :Respond_to_a_PowerApp_for_fail_for_no_access (Response);
    ...
    :Deleted_RPAs (Scope);
     if (condition) then (yes)
      ...
      :DeletedRPAs (Query);
      :Get_Actual__Deleted_RPAs (Scope);
       if (condition) then (yes)
        :Select_Actual__Deleted_RPAs (Select);
        :Parse_Actual__Deleted_RPAs (ParseJson);
       endif
       partition "Get_Actual__Deleted_RPAs" {
        :Select_Actual__Deleted_RPAs (Select);
        :Parse_Actual__Deleted_RPAs (ParseJson);
       }
      :Get_Inventory__Deleted_RPAs (Scope);
       if (condition) then (yes)
        :Select_Inventory__Deleted_RPAs (Select);
        :Parse_Inventory__Deleted_RPAs (ParseJson);
       endif
       partition "Get_Inventory__Deleted_RPAs" {
        :Select_Inventory__Deleted_RPAs (Select);
        :Parse_Inventory__Deleted_RPAs (ParseJson);
       }
     endif
     partition "Deleted_RPAs" {
      :Mark_deleted (Foreach);
       if (condition) then (yes)
        :Find_it_for_GUID (OpenApiConnection);
        :If_found_mark_deleted (If);
