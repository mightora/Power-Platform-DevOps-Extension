# Power Automate Flow: AdminSyncTemplatev4Solutions

**Generated on:** 2025-07-15 19:14:47
**Flow ID:** 000D3A341B0E
**Source File:** AdminSyncTemplatev4Solutions-838B0BC0-8494-EE11-BE37-000D3A341B0E.json
...
### Summary
- **Flow Name:** AdminSyncTemplatev4Solutions
- **Triggers:** 1
- **Actions:** 1 1 1 1 1 1
- **Connections:** 2
- **Parameters:** 4
...
@startuml
!theme plain
title Power Automate Flow: AdminSyncTemplatev4Solutions
...
:Initialize_solnsToInvenotry (InitializeVariable);
:Inventory_Solutions_for_this_envt (Scope);
 if (condition) then (yes)
  :Get_Basics (Scope);
   if (condition) then (yes)
    :Get_Solutions_and_Exit_if_no_Solutions_in_envt_or_if_no_permissions (Scope);
     if (condition) then (yes)
      :Count_Solutions (If);
       if (condition) then (yes)
        :Quit_if_no_Solutions (Terminate);
       endif
      :Terminate_if_no_access (Terminate);
      :List_Envt_Solutions (OpenApiConnection);
     endif
     partition "Get_Solutions_and_Exit_if_no_Solutions_in_envt_or_if_no_permissions" {
      :Count_Solutions (If);
       if (condition) then (yes)
        :Quit_if_no_Solutions (Terminate);
       endif
    ...
    :List_Inventory_Solutions (OpenApiConnection);
   endif
  ...
  :Look_for_Solutions_to_Inventory (Scope);
   if (condition) then (yes)
    :Look_for_new_Solutions (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_Solutions (Scope);
       if (condition) then (yes)
        :Select_Inventory__New_Solutions (Select);
        :Parse_Inventory__New_Solutions (ParseJson);
       endif
       partition "Get_Inventory__New_Solutions" {
        :Select_Inventory__New_Solutions (Select);
        :Parse_Inventory__New_Solutions (ParseJson);
       }
      :Get_Actual__New_Solutions (Scope);
       if (condition) then (yes)
        :Select_Actual__New_Solutions (Select);
        :Parse_Actual__New_Solutions (ParseJson);
       endif
       partition "Get_Actual__New_Solutions" {
        :Select_Actual__New_Solutions (Select);
        :Parse_Actual__New_Solutions (ParseJson);
       }
      :Add_new_ones_to_inventory_array (Scope);
       if (condition) then (yes)
        ...
        :Add_new_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_solnsToInvenotry__new_solutions (AppendToArrayVariable);
         endif
         repeat
          :Append_to_solnsToInvenotry__new_solutions (AppendToArrayVariable);
     ...
     partition "Look_for_new_Solutions" {
      :Get_Inventory__New_Solutions (Scope);
       if (condition) then (yes)
        :Select_Inventory__New_Solutions (Select);
        :Parse_Inventory__New_Solutions (ParseJson);
    ...
    :If_full_inventory_run (If);
     if (condition) then (yes)
      :Add_every_inventoried_portal_to_solnsToInvenotry (Foreach);
       if (condition) then (yes)
        :Append_to_solnsToInvenotry__full_inventory (AppendToArrayVariable);
       endif
       repeat
        :Append_to_solnsToInvenotry__full_inventory (AppendToArrayVariable);
       repeat while (more items)
      ...
      :Look_for_solutions_manually_flagged_for_inventory (Scope);
       if (condition) then (yes)
        :SolnsToInventory__ManualRequest (OpenApiConnection);
        :Add_manual_requests_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_solnsToInvenotry__Manual_Requests (AppendToArrayVariable);
         endif
         repeat
          :Append_to_solnsToInvenotry__Manual_Requests (AppendToArrayVariable);
         repeat while (more items)
       endif
       partition "Look_for_solutions_manually_flagged_for_inventory" {
        :SolnsToInventory__ManualRequest (OpenApiConnection);
        :Add_manual_requests_to_array (Foreach);
         if (condition) then (yes)
          :Append_to_solnsToInvenotry__Manual_Requests (AppendToArrayVariable);
      ...
      :Look_for_ones_modified_since_date (Scope);
       if (condition) then (yes)
        ...
        :Filter_to_solns_modified_since_Get_past_time (Query);
        :Add_each_recently_modified_to_array (Foreach);
         if (condition) then (yes)
          ...
          :Get_Solution_from_Inventory (OpenApiConnection);
         endif
         repeat
          :If_exists_in_inventory (If);
           if (condition) then (yes)
   ...
   partition "Look_for_Solutions_to_Inventory" {
    :Look_for_new_Solutions (Scope);
     if (condition) then (yes)
      :Get_Inventory__New_Solutions (Scope);
       if (condition) then (yes)
     ...
     if (condition) then (yes)
      ...
      :Object_Found_so_add_or_update_inventory (Scope);
       if (condition) then (yes)
        ...
        :Upsert_Record (If);
         if (condition) then (yes)
          :Upsert_Solution_Orphan (OpenApiConnection);
         endif
         else (no)
          :Upsert_Solution (OpenApiConnection);
         endif
        :Get_Solution_Publisher (Scope);
         if (condition) then (yes)
          :Get_Soln_Publisher (OpenApiConnection);
         endif
         partition "Get_Solution_Publisher" {
          :Get_Soln_Publisher (OpenApiConnection);
         }
       endif
 ...
 partition "Inventory_Solutions_for_this_envt" {
  :Get_Basics (Scope);
   if (condition) then (yes)
    :Get_Solutions_and_Exit_if_no_Solutions_in_envt_or_if_no_permissions (Scope);
     if (condition) then (yes)
...
| Initialize_solnsToInvenotry | InitializeVariable | Operation ID: b133bb41-c98a-45c4-a8f4-fbf40b3c3873 |
| Inventory_Solutions_for_this_envt | Scope | Operation ID: 7fabdbe3-cca0-4e6b-9f0b-b9ac39bea0b9 |
