# Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Solutions

**Generated on:** 2025-07-15 19:14:48
**Flow ID:** 000D3A341B0E
**Source File:** CLEANUPHELPER-CheckDeletedv4Solutions-3F5C06D9-8494-EE11-BE37-000D3A341B0E.json
...
### Summary
- **Flow Name:** CLEANUPHELPER-CheckDeletedv4Solutions
- **Triggers:** 1
- **Actions:** 1 1 1
- **Connections:** 3
- **Parameters:** 1
...
@startuml
!theme plain
title Power Automate Flow: CLEANUPHELPER-CheckDeletedv4Solutions
 ...
 if (condition) then (yes)
  ...
  :Run_Check_Deleted_Scope (Scope);
   if (condition) then (yes)
    ...
    :Get_Basics (Scope);
     if (condition) then (yes)
      :Get_Solutions_and_Exit_if_no_permissions (Scope);
       if (condition) then (yes)
       ...
       partition "Get_Solutions_and_Exit_if_no_permissions" {
        :List_Envt_Solutions (OpenApiConnection);
        :Terminate__no_access (Scope);
         if (condition) then (yes)
          :Respond_to_a_PowerApp_for_fail_for_no_access (Response);
    ...
    :Deleted_Solutions (Scope);
     if (condition) then (yes)
      :Get_Inventory__Deleted_Solutions (Scope);
       if (condition) then (yes)
        :Select_Inventory__Deleted_Solutions (Select);
        :Parse_Inventory__Deleted_Solutions (ParseJson);
       endif
       partition "Get_Inventory__Deleted_Solutions" {
        :Select_Inventory__Deleted_Solutions (Select);
        :Parse_Inventory__Deleted_Solutions (ParseJson);
       }
      :Get_Actual__Deleted_Solutions (Scope);
       if (condition) then (yes)
        :Select_Actual__Deleted_Solutions (Select);
        :Parse_Actual__Deleted_Solutions (ParseJson);
       endif
       partition "Get_Actual__Deleted_Solutions" {
        :Select_Actual__Deleted_Solutions (Select);
        :Parse_Actual__Deleted_Solutions (ParseJson);
       }
      :DeletedSolutions (Query);
